subroutine num_diff_q(the_model_aero)
  ...
  real(kind = 8), allocatable :: fae1(:), fae2(:), k_n(:,:), k_a(:,:), q0ae(:), v0ae(:), amatrix(:,:), oldcirculations(:), dfae(:,:), circulations(:)
...
!< calculating analytical tangent matrix
  ...
  call writeRealMatrixToFile(k_n,1000,'dfaex_n.dres')

  print*, maxval(abs(the_model_aero%dfae_ring(1:3*the_model_aero%tnrings,1:the_model_aero%tncoordinates)-k_n))
  return
end subroutine num_diff_q