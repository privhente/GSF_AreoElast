...
% =================================================================================================================
    ...
    %strfilename = ['v' num2str(model.environment.vinf) '_ns' num2str(model.components.blade.DeSiO.uvlm.M_aero) '_wo_twist_prebend'];
    ...
    path_DeSiO = {''};
    if isfield(model.simulationparameter,'path_DeSiO')
        path_DeSiO = model.simulationparameter.path_DeSiO;
    end
    
    ifort_version = {''};
    if isfield(model.simulationparameter,'ifort_version')
        ifort_version = model.simulationparameter.ifort_version;
    end
...
%                 remo_internal_const(end+1) = blades(i).grid.struc.matBeam.n_gl(1);
                simu_struct.constraints(nco).type  = 'revolutejoint_2';
                simu_struct.constraints(nco).nodes = [nhub blades(i).grid.struc.matBeam.n_gl(1)];
                simu_struct.constraints(nco).phi1  = [0,0,0];
            end
            nco0 = nco;
            for i = 1:n_blades
                nco  = nco0 + i;
                nhub = nnac + 1;
                simu_struct.constraints(nco).type  = 'rotation_local';
                simu_struct.constraints(nco).nodes = [blades(i).grid.struc.matBeam.n_gl(1) 0];
                simu_struct.constraints(nco).dir   = [0,0,1];
            end
...
if flag_controller == 1
    ...
    simu_controller.constraint_rotloc = size(monopile,1) + 3 + n_blades + [1:n_blades];
    simu_controller.currDir = currDir;
    simu_controller.strfilename = strfilename;
    
    % initializing load ID for rotor torque at hub node
    simu_struct.loads.sort      = 'constant';
    simu_struct.loads.intensity = 0.0;
    simu_struct.loads.duration  = 1.0
    simu_struct.loads.node      = nhub;
    simu_struct.loads.spatial   = [0,0,0,0,0,0];
    simu_struct.loads.material  = [0,0,0,0,0,0];
    
    % initializing load ID for rotor torque at hub node
    for i = 1:n_blades
        simu_struct.boundary12(i).sort         = 'delta';
        simu_struct.boundary12(i).intensity    = 0.0;
        simu_struct.boundary12(i).duration     = 0.0;
        simu_struct.boundary12(i).constraintID = simu_controller.constraint_rotloc(i);
        simu_struct.boundary12(i).file         = 'nofile';
    end
end 
% =================================================================================================================
%% ================================================================================================================
...
% =================================================================================================================
    ...
    if size(simu_fsi,2) ~= 0 
        ...
        if strncmp(os,'windows',length(os))
            fun_create_batchrunfile(path_DeSiO, path_mtee, path_fsi, path_struc, simu_fsi, simu_struct_static, simu_struct_dynamic, simu_struct_pertubation, flag_init_self_weight, flag_init_rotor_velocity, flag_aeroelastic_stability);
        else
            fun_create_bashrunfile(path_DeSiO, path_fsi, path_struc, simu_fsi, simu_struct_static, simu_struct_dynamic, simu_struct_pertubation, flag_init_self_weight, flag_init_rotor_velocity, flag_aeroelastic_stability);
        end
...
return
% =================================================================================================================
function fun_create_bashrunfile(path_DeSiO, path_fsi, path_struc, simu_fsi, simu_struct_static, simu_struct_dynamic, simu_struct_pertubation, flag_init_self_weight, flag_init_rotor_velocity, flag_aeroelastic_stability)
% write batch file for fsi calculation
    ...
    fclose(fid);
% =================================================================================================================
function fun_create_batchrunfile(path_DeSiO, path_mtee, path_fsi, path_struc, simu_fsi, simu_struct_static, simu_struct_dynamic, simu_struct_pertubation, flag_init_self_weight,flag_init_rotor_velocity, flag_aeroelastic_stability)
% write batch file for fsi calculation
    cd(path_fsi)
    ...
    elseif flag_aeroelastic_stability == 1
        ...
        fprintf(fid,'%s\n',['"' path_DeSiO '\DeSiO.exe' '"' ' | ' '"' path_mtee '\mtee.exe' '"' ' "' '%path_struc%' '\' simu_struct_pertubation.jobname '\output.txt"']);
        fprintf(fid,'%s\n',['REM Copying results files of dynamic struc simulation for init_rotor_vel to fsi simulation']);
