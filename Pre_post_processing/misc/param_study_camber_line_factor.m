...
% updated: 27.10.2022
% =================================================================================================================
function param_study_camber_line_factor()
% =================================================================================================================
...
arr_camber_line = [0.1:0.1:1.0];

for i = 1:length(arr_camber_line)

    camber_line_Factor = arr_camber_line(i);
    ...
    fid2 = fopen('test.yaml','w');
        while ~feof(fid1)
            ...
            strline = fgetl(fid1);
            TF_camber_line_factor = contains(strline,'camber_line_factor');
            TF_pitch = contains(strline,'pitch_angle');
            TF_omega = contains(strline,'init_rotor_velocity:','IgnoreCase',true);
            TF_flag  = contains(strline,'flag_init_rotor_velocity:','IgnoreCase',true);
            TF_vel = contains(strline,'vinf:','IgnoreCase',true);
            if TF_pitch == 1
                newstrline = ['pitch_angle: ' num2str(pitch)];
                fprintf(fid2,'  %s\n',newstrline);
            elseif TF_omega == 1 && TF_flag ~= 1
                newstrline = ['init_rotor_velocity: ' num2str(omega)];
                fprintf(fid2,'  %s\n',newstrline);
            elseif TF_vel == 1           
                newstrline = ['vinf: ' num2str(vel)];
                fprintf(fid2,'  %s\n',newstrline);
            elseif TF_camber_line_factor == 1
                newstrline = ['camber_line_factor: ' num2str(camber_line_Factor)];
                fprintf(fid2,'        %s\n',newstrline);
            else
                fprintf(fid2,'%s\n',strline);
            end
