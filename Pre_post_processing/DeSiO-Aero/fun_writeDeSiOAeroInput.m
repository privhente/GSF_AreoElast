...
fid = fopen('wakeinput.txt','w');
    ...
    for i = 1:n_wakes
        ...
        for j = 1:size(wake.wakes(i).inf,1)
            fprintf(fid,'%i\t',[wake.wakes(i).inf(j,:)]);
            fprintf(fid,'\n');
        end
    end
    fprintf(fid,'!!\n');
    ...
    for i = 1:nprop
        ...
        fprintf(fid,'%i\t%i\t%s',wake.property(i,1:2),str); 
        fprintf(fid,'\n');
    end
fclose(fid);
...
fid = fopen('simulationinput_aero.txt','w');
    ...
    strjobname     = 'solution'; if isfield(simu,'jobname'); strjobname = simu.jobname; end
    strprevjobname = ''; if isfield(simu,'prevjobname'); strprevjobname = simu.prevjobname; end
    ...
    strtype = 'aero'; if isfield(simu,'type'); strtype = simu.type; end
