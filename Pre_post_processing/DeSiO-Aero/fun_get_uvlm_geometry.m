...
% =================================================================================================================
function uvlm_ob = fun_get_uvlm_geometry(strName,model,airfoils,scale_opt,mesh_type_opt,mesh_factor_opt,blade_vl_model_opt)
% =================================================================================================================
    scale          = 1;
    mesh_type      = 0;
    mesh_factor    = 0;
    ...
    if nargin >= 7; blade_vl_model = blade_vl_model_opt; end

    % extracting uvlm data from WindIO
    uvlm_ob = fun_extract_uvlm_data(strName,model,airfoils,mesh_type,mesh_factor,blade_vl_model);
...
%     arr_twist      = -ones(mx+1,1)*pi/3;
    ...
    if isfield(uvlm_ob,'airfoil_orientation')
        arr_airfoil_orientation = uvlm_ob.airfoil_orientation;
    end
...
%         x_airf_w       = [xhi_airf_w(1,:)*rel_thickness; xhi_airf_w(2,:)*c; 0*O(3,:)];
        ...
        % this is only for assigning chord-length each node for fsi radius, see line 257
