...
if isfield(simu,'loads')
    fid = fopen('load12input.txt','w');
        ...
        for i = 1:size(simu.loads,2)
            ...
            if isfield(simu.loads(i),'sort'); strsort = simu.loads(i).sort; end
            if isfield(simu.loads(i),'intensity'); intensity = simu.loads(i).intensity; end
            if isfield(simu.loads(i),'duration'); duration = simu.loads(i).duration; end
            if isfield(simu.loads(i),'node'); node = simu.loads(i).node; end
            if isfield(simu.loads(i),'spatial'); spatial = simu.loads(i).spatial; end
            if isfield(simu.loads(i),'material'); material = simu.loads(i).material; end
            
            fprintf(fid,'%s\t',strsort);
            str = strrep(sprintf('%10.8e\t',intensity),'e','d'); fprintf(fid,'%s',str);
            str = strrep(sprintf('%10.8e\t',duration),'e','d'); fprintf(fid,'%s',str);
            fprintf(fid,'%i\t',node);
            str = strrep(sprintf('%10.8e\t',spatial),'e','d'); fprintf(fid,'%s',str);
            str = strrep(sprintf('%10.8e\t',material),'e','d'); fprintf(fid,'%s',str);
            fprintf(fid,'\n');
        end
    fclose(fid);
end
...
if isfield(simu,'boundary12')
    fid = fopen('boundary12input.txt','w');
        ...
        for i = 1:size(simu.boundary12,2)
            str = strrep(sprintf('%10.8e\t',[simu.boundary12(i).intensity,simu.boundary12(i).duration]),'e','d'); 
            fprintf(fid,'%s\t%s\t%i\t%s\n',simu.boundary12(i).sort, str, simu.boundary12(i).constraintID, simu.boundary12(i).file);
        end
    fclose(fid);
end
